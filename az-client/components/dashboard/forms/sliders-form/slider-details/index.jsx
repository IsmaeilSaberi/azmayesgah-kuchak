...
const SliderDetails = ({ sliderId }) => {
  ...
  const updater = (e) => {
    ...
    axios
      .post(
        `${process.env.NEXT_PUBLIC_SERVER_URL}/api/update-slider/${sliderId}`,
        formData,
        {
  ...
  useEffect(() => {
    axios
      .get(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/get-slider/${sliderId}`, {
        headers: { auth_cookie: auth_cookie },
      })
      .then((d) => {
        setFullData(d.data);
  ...
  const remover = (e) => {
    axios
      .post(
        `${process.env.NEXT_PUBLIC_SERVER_URL}/api/remove-slider/${sliderId}`,
        { item: 1 },
        {
          headers: { auth_cookie: auth_cookie },
        }
