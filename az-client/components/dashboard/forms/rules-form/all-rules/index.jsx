...
const AllRules = ({ setRuleDetailCtrl, setRandNumForRuleClick }) => {
  ...
  const [allRulesNumbers, setAllRulesNumbers] = useState(0);
  const paginate = 10;
  ...
  useEffect(() => {
    axios
      .get(
        `${process.env.NEXT_PUBLIC_SERVER_URL}/api/rules?pn=${pageNumber}&&pgn=${paginate}`,
        {
          headers: { auth_cookie: auth_cookie },
        }
      )
      .then((d) => {
        setRules(d.data.GoalRules);
        setBtnNumbers([
          ...Array(Math.ceil(d.data.AllRulesNumber / paginate)).keys(),
        ]);
        setAllRulesNumbers(d.data.AllRulesNumber);
      })
      .catch((err) => {
  ...
  return (
    <div className=" flex w-full flex-col gap-8">
      ...
      <div className="flex justify-center items-center gap-2">
        {filteredBtns[0] == -1 ? (
          <div className="flex justify-center items-center p-12">
            <Image alt="loading" width={40} height={40} src={"/loading2.svg"} />
          </div>
        ) : (
          filteredBtns.map((n, i) => (
            <button
              ...
              onClick={() => {
                n + 1 == pageNumber ? console.log("") : setRules([-1]);
                setPageNumber(n + 1);
                goToTop();
              }}
              key={i}
